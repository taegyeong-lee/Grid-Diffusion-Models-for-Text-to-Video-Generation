...
print(f'save in {parquet_folder_path}')
print(f'dataset from {folder_path}')

frame_input = cv2.imread(f'/home1/input_image.png')

for idx_ in tqdm.tqdm(range(order * len(frames)//2, (order + 1) * len(frames)//2, 500)):
    data = []
    index = idx_ // 500
    
    if os.path.isfile(f'{parquet_folder_path}/sd15_256_2x2_key_{index}.parquet'):
        print('file exist')
        continue
    ...
    pq.write_table(table, f'{parquet_folder_path}/sd15_256_2x2_key_{index}.parquet')